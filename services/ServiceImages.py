...
import evaluators.EvaluatorMultiAvgComp as EvaluatorMultiAvgComp
import evaluators.EvaluatorMultiAvgCompForTimestepLimit as EvaluatorMultiAvgCompForTimestepLimit
import ServiceSavedModel
from enums.EnumMetrics import Metrics
import ServiceMetric
import ServiceGeneral
import animator.AnimatorScatter as AnimatorScatter
import animator.AnimatorScatterMulti as AnimatorScatterMulti
...
def createMultiPlotFromScratch(xLabels, yLabels, data, index, title=None, xAxisLabel=None, yAxisLabel=None, savePath=None, 
                               xlim =(0,1000), ylim=None, legendRows=1, fontsize=11, colourBackgroundForTimesteps=[None, None]):
    """
        ...
        for b in range(nColsOuter):
            ...
            for x in range(nRowsInner):
                for y in range(nColsInner):
                    ...
                    if nRowsInner == 1 and nColsInner == 1:
                        ax = axes
                    elif nRowsInner == 1:
                        ax = axes[y]
                    elif nColsInner == 1:
